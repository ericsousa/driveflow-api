...
export class NotaFiscalService {
    ...
    private async verificarEstoqueCarro(id_carro: number): Promise<void> {
        ...
        if (estoques.length === 0) {
            throw new NotFoundError('Carro não encontrado em estoque.');
        }

        if (estoques[0].quantidade <= 0) {
            throw new BusinessError('Carro sem estoque disponível para venda.');
        }
    }
    ...
    private async decrementaEstoqueCarro(id_carro: number): Promise<void> {
        ...
        if (!updated) {
            throw new BusinessError('Não foi possível atualizar o estoque do carro.');
        }
    }
    ...
    private async verificarExistenciaCliente(id_cliente: number): Promise<void> {
        ...
        if (!cliente) {
            throw new NotFoundError('Cliente relacionado à nota fiscal não encontrado.');
        }
    }
    ...
    private async verificarExistenciaVendedor(id_vendedor: number): Promise<void> {
        ...
        if (!vendedor) {
            throw new NotFoundError('Vendedor relacionado à nota fiscal não encontrado.');
        }
    }
    ...
    private async verificarExistenciaCarro(id_carro: number): Promise<void> {
        ...
        if (!carro) {
            throw new NotFoundError('Carro relacionado à nota fiscal não encontrado.');
        }
    }
    ...
    private async validaNotaUnica(numero_nota: string): Promise<void> {
        ...
        if (notaExistente) {
            throw new ConflictError('Número da nota fiscal já existe. Deve ser único.');
        }
    }
    ...
    private validaDataEmissao(data_emissao: string | Date) {
        ...
        if (isNaN(dataAtual.getTime())) {
            throw new ValidationError('Data de emissão inválida.');
        }
        if (dataAtual > new Date()) {
            throw new ValidationError('Data de emissão não pode ser futura.');
        }
    }
    ...
    private validarCamposObrigatorios(data: any) {
        if (!data.numero_nota) {
            throw new ValidationError('Campo numero_nota é obrigatório');
        }
        if (!data.data_emissao) {
            throw new ValidationError('Campo data_emissao é obrigatório');
        }
        if (data.valor_total === undefined || data.valor_total === null) {
            throw new ValidationError('Campo valor_total é obrigatório');
        }
        if (data.id_cliente === undefined || data.id_cliente === null) {
            throw new ValidationError('Campo id_cliente é obrigatório');
        }
        if (data.id_vendedor === undefined || data.id_vendedor === null) {
            throw new ValidationError('Campo id_vendedor é obrigatório');
        }
